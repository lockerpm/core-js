...
import { EncryptionType } from '../../src/enums/encryptionType'
import { KdfType } from '../../src/enums/kdfType'

import { EncArrayBuffer } from '../../src/models/domain/encArrayBuffer'
import { EncryptedObject } from '../../src/models/domain/encryptedObject'
import { EncString } from '../../src/models/domain/encString'
import { SymmetricCryptoKey } from '../../src/models/domain/symmetricCryptoKey'
import { ProfileOrganizationResponse } from '../../src/models/response/profileOrganizationResponse'

import { CryptoService as CryptoServiceAbstraction } from '../../src/abstractions/crypto.service'
...
import { EEFLongWordList } from '../../src/misc/wordlist'
...
export class CryptoService implements CryptoServiceAbstraction {
    ...
    if (key != null) {
      if (!(await this.shouldStoreKey())) {
        this.logService.warning(
          'Throwing away stored key since settings have changed'
        )
        this.secureStorageService.remove(Keys.key)
        return null
      }

      const symmetricKey = new SymmetricCryptoKey(
        Utils.fromB64ToArray(key).buffer
      )
  ...
  async getEncKey (key: SymmetricCryptoKey = null): Promise<SymmetricCryptoKey> {
    ...
    if (encKeyCipher.encryptionType === EncryptionType.AesCbc256_B64) {
      ...
      encKeyCipher.encryptionType === EncryptionType.AesCbc256_HmacSha256_B64
    ) {
  ...
  async getPublicKey (): Promise<ArrayBuffer> {
    ...
    this.publicKey = await this.cryptoFunctionService.rsaExtractPublicKey(
      privateKey
    )
    return this.publicKey
  }
  ...
  async getPrivateKey (): Promise<ArrayBuffer> {
    ...
    const encPrivateKey = await this.storageService.get<string>(
      Keys.encPrivateKey
    )
    if (encPrivateKey == null) {
      return null
    ...
    this.privateKey = await this.decryptToBytes(
  ...
  async getFingerprint (
    ...
    const keyFingerprint = await this.cryptoFunctionService.hash(
      publicKey,
      'sha256'
    )
  ...
  async getOrgKeys (): Promise<Map<string, SymmetricCryptoKey>> {
    ...
    const orgKeys: Map<string, SymmetricCryptoKey> = new Map<
      string,
      SymmetricCryptoKey
    >()
    let setKey = false
  ...
  async toggleKey (): Promise<any> {
    ...
    const option = await this.storageService.get(
      ConstantsService.vaultTimeoutKey
    )
    const biometric = await this.storageService.get(
      ConstantsService.biometricUnlockKey
    )
    if (
      !biometric &&
      this.platformUtilService.supportsSecureStorage() &&
  ...
  async encryptToBytes (
    ...
    encBytes.set(
      new Uint8Array(encValue.data),
      1 + encValue.iv.byteLength + macLen
    )
  ...
  async rsaEncrypt (
    ...
    const encBytes = await this.cryptoFunctionService.rsaEncrypt(
      data,
      publicKey,
      'sha1'
    )
    return new EncString(
      ...
      Utils.fromBufferToB64(encBytes)
    )
  }
  ...
  async decryptToBytes (
    ...
    const mac = encString.mac
      ? Utils.fromB64ToArray(encString.mac).buffer
      : null
  ...
  async validateKey (key: SymmetricCryptoKey) {
    try {
      ...
      const privateKey = await this.decryptToBytes(
        new EncString(encPrivateKey),
        encKey
      )
  ...
  private async shouldStoreKey () {
    const vaultTimeout = await this.storageService.get<number>(
      ConstantsService.vaultTimeoutKey
    )
    ...
    const biometricsEnabled =
      biometricUnlock && this.platformUtilService.supportsSecureStorage()
  ...
  private async aesDecryptToBytes (
    ...
    if (theKey.macKey != null && mac != null) {
      ...
      const macsMatch = await this.cryptoFunctionService.compare(
        mac,
        computedMac
      )
      if (!macsMatch) {
  ...
  private async getKeyForEncryption (
    key?: SymmetricCryptoKey
  ): Promise<SymmetricCryptoKey> {
    if (key != null) {
      return key
  ...
  async decryptData (otp: any, qr: any) {
    ...
    const dataBuffer = await this.cryptoFunctionService.aesDecrypt(
      encryptB64,
      iv,
      keyBuff
    )
    ...
    const key = new SymmetricCryptoKey(
      Utils.fromB64ToArray(keyB64).buffer,
      parseInt(encType)
    )
