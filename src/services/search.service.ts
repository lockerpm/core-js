...
import { SendView } from '../../src/models/view/sendView'
...
export class SearchService implements SearchServiceAbstraction {
  ...
  constructor(
    ...
    if (['zh-CN', 'zh-TW'].includes(i18nService.locale)) {
  ...
  searchSends(sends: SendView[], query: string) {
    ...
    return sends.filter(s => {
      ...
      //   s.file.fileName.toLowerCase().includes(query)
      // ) {
      //   return true
      // }
    })
