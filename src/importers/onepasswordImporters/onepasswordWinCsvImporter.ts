...
export class OnePasswordWinCsvImporter
  ...
  setCipherType (value: any, cipher: CipherView) {
    ...
    if (
      !this.isNullOrWhitespace(
        this.getPropByRegexp(value, /name \d+: first name/i)
      ) ||
      !this.isNullOrWhitespace(
        this.getPropByRegexp(value, /name \d+: initial/i)
      ) ||
      !this.isNullOrWhitespace(
        this.getPropByRegexp(value, /name \d+: last name/i)
      ) ||
      !this.isNullOrWhitespace(
        this.getPropByRegexp(value, /internet \d+: email/i)
      )
    ) {
  ...
  setCreditCardExpiry (context: CipherImportContext) {
    if (
      ...
      if (
        context.cipher.card.expMonth[0] === '0' &&
        context.cipher.card.expMonth.length === 2
      ) {
        context.cipher.card.expMonth = context.cipher.card.expMonth.substr(1, 1)
      }
      context.cipher.card.expYear =
        expSplit[2].length > 4 ? expSplit[2].substr(0, 4) : expSplit[2]
      return true
    }
    return false
  }
