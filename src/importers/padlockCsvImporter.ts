...
import { CollectionView } from '../../src/models/view/collectionView'
  ...
  parse (data: string): Promise<ImportResult> {
    ...
    results.forEach(value => {
      ...
      for (let i = 2; i < value.length; i++) {
        ...
        if (this.usernameFieldNames.includes(header)) {
          cipher.login.username = value[i]
        } else if (this.passwordFieldNames.includes(header)) {
          cipher.login.password = value[i]
        } else if (this.uriFieldNames.includes(header)) {
